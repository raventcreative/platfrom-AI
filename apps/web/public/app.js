...
function genScript(c, daily, seedN) {
  ...
  scenes = scaleDur(scenes, target).map((s, i) => ({ scene: i + 1, ...s })); // skala durasi ke target + nomori adegan
  return {
...
// GEN: rencana konten 7 hari, campuran pillar 80/20 (5 memberi : 2 meminta)
...
function suggestCompetitors(c, extra) {
  ...
  const top = scored.filter(x => x.d === topDomain).slice(0, 3).map(x => x.h);
  ...
  return [...new Set([...top, ...(CATEGORY_FALLBACK[c.category] || [])])].slice(0, 3);
}
...
function renderRiset(body, c) {
  ...
  cmpCard.append(el('p', { class: 'muted', style: 'margin:0 0 12px; font-size:14px;', html: 'Isi <b>@handle kompetitor aslimu</b> → kami tampilkan <b>preview feed asli langsung dari Instagram</b> + analisis pola. Belum tahu mau bandingin siapa? Klik <b>Analisis pola otomatis</b> buat lihat pola konten umum di niche-mu (bukan akun asli).' }));
  cmpCard.append(el('div', { class: 'hint', style: 'margin-bottom:2px;', html: 'Preview IG butuh <b>internet</b> & handle asli. Tempel caption mereka (opsional) buat skor lebih presisi.' }));
  ...
  // ── CONNECT: kompetitor di Riset ↔ kolom Kompetitor di Profil Brand (comp1/2/3) ──
  const profileComps = () => (Array.isArray(c.competitors) && c.competitors.length ? c.competitors : [c.comp1, c.comp2, c.comp3]).map(x => (x || '').trim()).filter(Boolean);
  // tulis handle Riset → balik ke Profil Brand (comp1/2/3 + competitors) biar selalu sinkron
  const syncToProfile = () => { const hs = saved.comps.map(x => (x.handle || '').trim()); c.comp1 = hs[0] || ''; c.comp2 = hs[1] || ''; c.comp3 = hs[2] || ''; c.competitors = hs.filter(Boolean); saveClient(c); };
  // isi otomatis handle saran (akun asli, dicocokkan ke jasa/produk brand) — lalu tulis ke Profil
  const fillSuggested = () => { const s = suggestCompetitors(c); saved.comps = (s.length ? s : ['']).slice(0, 3).map(h => ({ handle: h, raw: '' })); saved.suggestedApplied = true; syncToProfile(); persist(); return s; };
  ...
  if (prof.length) {
    ...
    saved.comps = prof.slice(0, 3).map(h => ({ handle: h, raw: prev[h.replace(/^@/, '').toLowerCase()] || '' }));
  ...
  function regen() { fillSuggested(); drawComps(); refreshSearchNote(); toast('Saran kompetitor diisi ✓', 'ok'); }
  ...
  cmpCard.append(el('div', { class: 'row', style: 'margin-top:14px; flex-wrap:wrap;' }, [
    el('button', { class: 'primary', textContent: '📸 Bandingkan + preview IG', onclick: () => runCompare(false) }),
...
function renderCompare(d, c) {
  ...
  card.append(el('div', { class: 'verdict' }, [
    ...
    el('div', { class: 'tiny', style: 'margin-top:4px;', textContent: '▲/▼ = kolom kamu dibanding ' + vs + '.' + (d.competitors.some(cp => cp.archetype) ? ' Kompetitor di sini = POLA niche, bukan akun asli — tempel caption asli mereka buat angka nyata.' : '') }),
  ].filter(Boolean)));
  ...
  d.competitors.forEach((cp) => {
    ...
    if (!cp.archetype) card.append(igEmbedCard(cp.handle)); // preview IG ASLI untuk handle beneran
  ...
  // ── PREVIEW IG ASLI dari handle yang kamu masukin (embed resmi Instagram) ──
  card.append(el('div', { class: 'divider' }));
  card.append(el('b', { style: 'display:block;', textContent: '📸 Preview Instagram kompetitor (asli)' }));
  if (d.realHandles && d.realHandles.length) {
    card.append(el('div', { class: 'tiny', style: 'margin:4px 0 10px;', textContent: 'Profil & feed asli langsung dari Instagram — bukan gambar buatan.' }));
    const grid = el('div', { class: 'igembed-grid' });
    d.realHandles.forEach(h => grid.append(igEmbedCard(h)));
    card.append(grid);
  } else {
    card.append(el('div', { class: 'hint', style: 'margin-top:4px; margin-bottom:12px;', html: 'Belum ada handle. Cari kompetitor aslimu lewat pencarian Instagram di bawah → salin @handle-nya → tempel di kolom kompetitor → bandingkan lagi. Feed aslinya bakal tampil di sini.' }));
    card.append(renderIgSearchPanel(c));
  }
  if (d.gaps.length) {
