...
// handle kompetitor tipikal per niche (buat mode auto-deteksi)
const COMP_HANDLES = {
  skincare: ['@glowrival.id', '@skinsecret.co', '@beautyhaul.id'],
  fnb: ['@dapurviral.id', '@kulinerhits.co', '@rasajuara.id'],
  fashion: ['@outfitdaily.id', '@stylehunt.co', '@wardrobe.hits'],
  service: ['@jasapro.id', '@solusicepat.co', '@partnerbisnis.id'],
  edu: ['@belajarcepat.id', '@kelaspintar.co', '@skillup.id'],
  other: ['@rivalbrand.id', '@pesaing.co', '@kompetitorhits.id'],
};
// Deteksi kompetitor OTOMATIS: rakit 3 arketipe akun yang lazim di niche brand
// (Si Jualan Terus / Si Estetik / Si Edukatif). Caption tiap akun disintesis
// deterministik dari bank kategori supaya analisisnya realistis & bisa direproduksi.
// Dipakai kalau user nggak nempel caption kompetitor sendiri.
function detectCompetitors(c) {
  ...
  const handles = COMP_HANDLES[c.category] || COMP_HANDLES.other;
  ...
  return [
    { handle: handles[0], raw: hardsell.join('\n---\n') },
    { handle: handles[1], raw: aesthetic.join('\n---\n') },
    { handle: handles[2], raw: edu.join('\n---\n') },
  ];
}
...
function compareReport(c, myRaw, comps, autoDetected) {
  const f = factsOf(c);
  ...
  return {
    type: 'compare', mine, mineAuto, competitors, autoDetected: !!autoDetected, gaps: gaps.map(g => g.label + ' — belum ada yang menggarap ini'), hook_bank, action_ideas: action_ideas.slice(0, 5),
    learning_note: autoDetected
      ? 'Kompetitor di atas dideteksi otomatis: 3 arketipe akun yang paling sering muncul di niche-mu (si jualan terus, si estetik, si edukatif). Pola & celahnya nyata untuk niche ini — kalau mau lebih presisi, tempel caption pesaing aslimu di kolomnya. Prinsipnya: tiru polanya, jangan kalimatnya; menang di celah yang mereka tinggalkan.'
...
function renderRiset(body, c) {
  ...
  cmpCard.append(el('p', { class: 'muted', style: 'margin:0 0 12px; font-size:14px;', html: 'Nggak perlu masukin apa-apa — klik <b>Deteksi kompetitor otomatis</b> dan kami rakit 3 arketipe pesaing tipikal di niche-mu, lalu bandingkan & carikan celahmu. Mau lebih presisi? Tempel caption pesaing aslimu (opsional) di kolom bawah.' }));
  cmpCard.append(el('div', { class: 'hint', style: 'margin-bottom:2px;', html: 'Isi kompetitor <b>opsional</b>. Kosongkan = auto-deteksi dari kategori brand-mu.' }));
  ...
  function drawComps() {
    ...
    saved.comps.forEach((cp, i) => {
      ...
      const h = el('input', { type: 'text', placeholder: '@handle_kompetitor (opsional)', value: cp.handle || '', style: 'margin-bottom:8px;' });
  ...
  const runCompare = forceAuto => {
    ...
    const rep = compareReport(c, saved.myPosts || '', comps, auto);
    cmpOut.innerHTML = '';
    cmpOut.append(renderCompare(rep, c));
    cmpOut.scrollIntoView({ behavior: 'smooth', block: 'start' });
    toast(auto ? 'Kompetitor terdeteksi otomatis ✓' : 'Perbandingan siap ✓', 'ok');
  };
  cmpCard.append(el('div', { class: 'row', style: 'margin-top:14px;' }, [
    el('button', { class: 'primary', textContent: '🔍 Deteksi kompetitor otomatis', onclick: () => runCompare(true) }),
    el('button', { textContent: '🥊 Bandingkan caption yang ditempel', onclick: () => runCompare(false) }),
...
function renderCompare(d, c) {
  ...
  if (d.autoDetected) card.append(el('div', { class: 'hint', style: 'margin:0 0 12px;', html: '🔍 <b>Kompetitor terdeteksi otomatis</b> — 3 arketipe pesaing tipikal di niche-mu. Tempel caption pesaing aslimu di kolom di atas untuk hasil lebih presisi.' }));
  const tbl = el('table', { class: 't' });
